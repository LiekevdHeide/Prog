...
void proportionalSwitch(ScheduleAndFlows &Schedule, size_t t, size_t od, const vector<vector<double>> &pathTimes) {
	...
	for (size_t p = 0; p < Schedule.numAvailableRoutes[t + 1][od]; ++p) {
		...
		for (size_t r = 0; r < Schedule.numAvailableRoutes[t + 1][od]; ++r) { // if not available flow = 0
			if (r != p) {
				Schedule.pathFlow[t + 1][od][Schedule.availableRoutes[t + 1][od][p]] += 0.001 * (Schedule.pathFlow[t][od][Schedule.availableRoutes[t + 1][od][r]] * max(pathTimes[od][r] - pathTimes[od][p], 0.0) - Schedule.pathFlow[t][od][Schedule.availableRoutes[t + 1][od][p]] * max(pathTimes[od][p] - pathTimes[od][r], 0.0));
				...
				/*if (t == 32 || t == 33) {
					cout << Schedule.availableRoutes[t + 1][od][r] << "->" << Schedule.availableRoutes[t + 1][od][p] << ' ' << 0.001 * (Schedule.pathFlow[t][od][Schedule.availableRoutes[t + 1][od][r]] * max(pathTimes[od][r] - pathTimes[od][p], 0.0) - Schedule.pathFlow[t][od][Schedule.availableRoutes[t + 1][od][p]] * max(pathTimes[od][p] - pathTimes[od][r], 0.0));
	
					cout << ' ' << 0.001 * Schedule.pathFlow[t][od][Schedule.availableRoutes[t + 1][od][r]] << " max " << pathTimes[od][r] << '-' << pathTimes[od][p] << " 0) - " << 0.01 * Schedule.pathFlow[t][od][Schedule.availableRoutes[t + 1][od][p]] << " * max(" << pathTimes[od][p] << '-' << pathTimes[od][r] << " , 0)";
					cout << " res:" << Schedule.pathFlow[t + 1][od][Schedule.availableRoutes[t + 1][od][p]] << " trial: " << Schedule.pathFlow[t][od][Schedule.availableRoutes[t + 1][od][r]] << " trial: " << Schedule.pathFlow[t][od][Schedule.availableRoutes[t + 1][od][p]] << '\n';
				}*/
			}
		}
	}
