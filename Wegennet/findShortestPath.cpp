...
using namespace std;
size_t findShortestPath(RoadNetwork Roads, size_t od, vector<double> pathFlows) { //ASSUMES T=0!!!
	...
	double shortestPathLength{ Roads.pathTravelTime(Roads.ODpaths[od][0], pathFlows[0]) };//err

	//cout << shortestPathLength << ' ';
	for (size_t p = 1; p < Roads.numberODpaths[od]; ++p) {
		//calculate path length
		pathLength = Roads.pathTravelTime(Roads.ODpaths[od][p],pathFlows[p]);
		//cout << pathLength << ' ';
		//if < shortest: update
